...
static std::string currentChatUser = "all";


void motifyUserList(const std::string& str) {
    userList.clear();
    std::stringstream ss(str);
    std::string item;
    //std::cout <<"user list: "<< str << " ";
...
int main(int, char**)
{
    ...
    while (!done)
    {
		...
		//if not login, show login window
        if (!login)
        {
            ...
            ImGui::Begin("Login");
            {
                ...
                ImGui::Dummy(ImVec2(0, win_height * 0.35f));

                ImGui::SetNextItemWidth(-FLT_MIN);
                static char _username[128] = "";
                ImGui::InputTextWithHint("##input", "please enter your username", _username, IM_ARRAYSIZE(_username));
                ...
                {
                    login = true;
                    client.connect_to_server(_username);
                    username = _username;
                    Sleep(300);
                    //update user list
                    std::string str = client.updateUserList();
        ...
        if (login)
        {
			//check if need to update user list
            if(client.isNewUser())
            {
                std::string str = client.updateUserList();
                //std::cout << "other user client:" << str << std::endl;
                if (!str.empty())
                {
                    motifyUserList(str);
                }
            ...
            // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
            {
                ...
                ImGui::Begin("Chat Client");                          // Create a window called "Hello, world!" and append into it.
                ...
                //user list
                ImGui::BeginChild("user panel", ImVec2(300, 600), true);
                {
                    ImGui::Text("Users:");
                    ImGui::Separator();

					//username is in the top of user list
                    ImGui::Text(username.c_str());
                    //user list show as buttons, for private chat
                    for(auto& u : userList)
                    {
                        if (u != username)
                        {
                    ...
                    //if (ImGui::Button("Fox"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
                    //{
                    //    show_another_window = true;
                    //}
                    //if (ImGui::Button("Lion"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
                    //{
                    //    show_another_window = true;
                    //}
                    //if (ImGui::Button("Rabbit"))                            // Buttons return true when clicked (most widgets return true when edited/activated)
                    //{
                    //    show_another_window = true;
                    //}
                }
				...
				//chat panel
                ImGui::BeginChild("chat panel", ImVec2(0, 600), true);
                {
                    //ImGui::Text("Fox:Hello!");
                    auto msgs = client.getMessage();
                    for (auto& m : msgs)
                    {
                        ImGui::Text("%s: ", m.sender.c_str());
                        ImGui::SameLine();
				...
				//input panel
                static char buf[128] = "";
                ImGui::SetNextItemWidth(863);
				//send message when press enter
                ImGui::InputTextWithHint("##input", "please enter message...", buf, IM_ARRAYSIZE(buf));
     //           if (ImGui::InputTextWithHint("##input", "please enter message...", buf, IM_ARRAYSIZE(buf)), ImGuiInputTextFlags_EnterReturnsTrue)
     //           {
					////client.sendMessage(buf,"all");
     //               //std::cout << "test";
     //               //buf[0] = '\0';
     //           }
            ...
            if (show_another_window)
            {
				...
				std::string title = "Private Chat with " + currentChatUser;
                ImGui::Begin(title.c_str(), &show_another_window);   // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
                {
                    ImGui::BeginChild("chat panel", ImVec2(0, 400), true);
                    {
                        //ImGui::Text("Roar! I gonna hunt you down! Fox!");
                        auto msgs = client.getPrivateMessage(username, currentChatUser);;
                        for (auto& m : msgs)
                        {
                            ImGui::Text("%s: ", m.sender.c_str());
                            ImGui::SameLine();
                ...
                {
                    client.sendMessage(buf, currentChatUser);
                    //std::cout << "receiver:" << currentChatUser<< std::endl;
                    buf[0] = '\0';
                }
